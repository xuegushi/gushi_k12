...
| macOS (Apple Silicon) | [下载 .dmg](https://github.com/xuegushi/gushi_k12/releases/tag/v0.1.12) |
| 桌面版 (v0.1.12) | [GitHub Releases](https://github.com/xuegushi/gushi_k12/releases/tag/v0.1.12) |
...
- **2203 首诗词**（含课内 + 课外补充，热门诗人至少 5 首），诗按标点独立分行，词/文言文保持原始结构
- **283 位作者简介**（含生平介绍、代表作品）
- **1441 个 AI 知识点**（K1-9 全覆盖）
- **课外诗单**：唐诗三百首（318首）、宋词三百首（293首）、古诗三百（262首）、古诗十九首（19首）
- 课内诗词 **译文覆盖率 84%**（从 gushi.poem_detail 批量导入），注释/赏析同步补齐
...
| 诗词详情 | 原文/知识双栏，译文/注释/赏析/创作背景/作者信息/知识点，Markdown 渲染 |
| 拼音标注 | 全文拼音（pinyin-pro），多音字自动识别 |
| 朗读 | Web Speech API 中文朗读 |
| 背诵模式 | 艾宾浩斯遗忘曲线复习（7阶段），挖字辅助记忆（全隐/首字/末字/随机） |
| 学习计划 | 按年级/学期自动生成，勾选完成跟踪进度，弹窗标题/朝代/作者居中展示，各阶段诗词数量统计，学习流程说明弹窗 |
| AI 助手 | 通义千问 / DeepSeek / Kimi / MiniMax，用户自配 Key |
| AI 生成 | 缺失的译文/注释/赏析/创作背景可一键 AI 生成并持久化到 IndexedDB |
...
| 诗词找茬 | 展示含形近字替换的诗句，找出被替换的错字（3-5 处），庆祝动画 |
| 诗词接龙 | 显示上句/下句，四选一配对，顺接/逆接双模式，庆祝动画 |
| 诗词雨 | Canvas 矩阵雨动画，逐列飘落诗句，悬停展示详情 |
| 诗词排序 | 拖拽标点切分的片段，还原诗句顺序，庆祝动画 |
| 诗词填空 | 从形近字混淆的候选池选字填入诗句空白处，庆祝动画 |
| 飞花令 | 输入包含指定主题字的诗句（8 类 158 个字库） |
| 诗词连连看 | 多首诗混合配对上下句，匹配后显示诗名，庆祝动画 |
| 诗词拼图 | N×N 文字矩阵候选池，含干扰字，按原诗行填入，庆祝动画，支持分享导出图片及背景色切换 |
| 猜猜他是谁 | 根据诗人足迹线索（人生足迹、游历记录、漫游路线）猜诗人身份，四选一答题，3 条扣分提示，支持分享导出图片 |
| 游戏音效 | 全局静音按钮，正误音效反馈 |
